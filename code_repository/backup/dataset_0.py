    ...
    def __init__(self, metadata_path, data_transform=False):
        ...
        for i in range(self.raw_data.shape[0]):
            ...
            for j in range(img.shape[2]):
                self.img_list.append(cv2.normalize(src=img[..., j], dst=None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U))
                self.label_list.append(label[...,j].astype(np.uint8))
